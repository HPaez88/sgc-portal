    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-wrap justify-between gap-4 items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-[#002855]">Control Documental</h2>
      ...
      {docsFiltrados.length === 0 ? (
         <div className="text-center py-16 text-slate-400 bg-white rounded-xl shadow-sm border border-slate-200">
           <p className="text-5xl mb-4 opacity-50">📂</p>
           <p className="text-lg font-bold text-slate-600">No hay documentos</p>
          ...
          {docsFiltrados.map(doc => (
            <div key={doc.id} className="bg-white p-5 rounded-2xl border border-slate-200 hover:shadow-lg transition-all group relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-4">
                <span className="text-xs font-bold bg-slate-100 text-slate-600 border border-slate-200 px-2.5 py-1 rounded-md">{doc.tipo}</span>
              ...
              <div className="text-sm text-slate-500 space-y-1.5 mb-5 bg-slate-50 p-3 rounded-lg border border-slate-100">
                <p className="flex justify-between"><span className="font-medium text-slate-400">Área:</span> <span className="text-right font-medium text-slate-700">{doc.area}</span></p>
                <p className="flex justify-between"><span className="font-medium text-slate-400">Versión:</span> <span className="font-medium text-slate-700">v{doc.version}</span></p>
                <p className="flex justify-between"><span className="font-medium text-slate-400">Fecha:</span> <span className="font-medium text-slate-700">{doc.fecha}</span></p>
                <p className="flex justify-between"><span className="font-medium text-slate-400">Autor:</span> <span className="text-right font-medium text-slate-700 truncate max-w-[120px]" title={doc.autor}>{doc.autor}</span></p>
              </div>
              
              <div className="flex gap-3">
                <button onClick={() => exportarDocumento(doc)} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-sm font-bold text-slate-600 bg-slate-50 border border-slate-200 rounded-xl hover:bg-cyan-50 hover:text-cyan-600 hover:border-cyan-200 transition-colors">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-slide-up">
            ...
            <div className="p-6 space-y-5">
              <div>
                <label className="block text-sm font-bold text-slate-600 mb-1.5">Título del Documento</label>
              ...
              <div className="grid grid-cols-2 gap-5">
                <div>
                  <label className="block text-sm font-bold text-slate-600 mb-1.5">Tipo</label>
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-600 mb-1.5">Versión Inicial</label>
              ...
              <div>
                <label className="block text-sm font-bold text-slate-600 mb-1.5">Área Propietaria</label>
              ...
              <div>
                <label className="block text-sm font-bold text-slate-600 mb-1.5">Autor / Elaboró</label>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex gap-3">
              <button onClick={() => setMostrarModal(false)} className="flex-1 px-4 py-2.5 border border-slate-200 bg-white font-bold text-slate-600 rounded-lg hover:bg-slate-50 transition-colors">Cancelar</button>
