...
export default function GestorAprobacionesView({ 
  ...
  const getPrioridadColor = (p) => {
    ...
    return colors[p] || 'bg-slate-100 text-slate-600 border-slate-200';
  };
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="p-4 text-sm font-bold text-slate-600">Documento</th>
                  <th className="p-4 text-sm font-bold text-slate-600">Tipo</th>
                  <th className="p-4 text-sm font-bold text-slate-600">Área</th>
                  <th className="p-4 text-sm font-bold text-slate-600">Fecha Envío</th>
                  <th className="p-4 text-sm font-bold text-slate-600">Prioridad</th>
                  <th className="p-4 text-sm font-bold text-slate-600">Estado</th>
                  <th className="p-4 text-sm font-bold text-slate-600 text-center">Acciones SGC</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {appsFiltradas.map(app => (
                  <tr key={`${app.tipo}-${app.id_original}`} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="p-4 font-mono font-bold text-[#002855] text-sm">
                      {app.documento}
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-sm text-slate-600 font-medium">{app.area}</td>
