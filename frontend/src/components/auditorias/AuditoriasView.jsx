                  ...
                  {informesFiltrados.length === 0 ? (
                    ...
                    informesFiltrados.map((inf, i) => (
                      <div key={i} className="bg-white p-4 rounded-xl border border-slate-200 flex items-start gap-4 hover:shadow-md transition-shadow group">
                        ...
                        <button className="text-slate-500 hover:text-cyan-600 p-2 hover:bg-cyan-50 rounded-lg transition-colors">
                          <Download size={18} />
                        </button>
                      </div>
                    ))
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50/50">
              <tr>
                <th className="p-4 text-sm font-bold text-slate-700">Número</th>
                <th className="p-4 text-sm font-bold text-slate-700">Área Auditada</th>
                <th className="p-4 text-sm font-bold text-slate-700">Tipo</th>
                <th className="p-4 text-sm font-bold text-slate-700">Fechas Programadas</th>
                <th className="p-4 text-sm font-bold text-slate-700 text-center">Hallazgos</th>
                <th className="p-4 text-sm font-bold text-slate-700">Estado</th>
                <th className="p-4 text-sm font-bold text-slate-700 text-center">Acciones</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {safeAuditorias.length === 0 ? (
                ...
                <tr key={aud.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="p-4 font-medium text-slate-800">{aud.area}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-center">
                    <button className="text-slate-500 hover:text-cyan-600 p-2 hover:bg-cyan-50 rounded-lg transition-colors inline-flex">
                      <Eye size={18} />
                    </button>
                  </td>
                </tr>
      ...
      {mostrarModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-slide-up">
            <div className="px-6 py-4 bg-gradient-to-r from-[#002855] to-[#00152e] flex justify-between items-center">
              <h3 className="font-bold text-[#002855] flex items-center gap-2"><Plus size={18} /> Programar Auditoría</h3>
            </div>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end">
              <button onClick={() => setMostrarModal(false)} className="px-6 py-2.5 border border-slate-200 bg-white font-bold text-slate-700 rounded-lg hover:bg-slate-50 transition-colors">Cerrar</button>
            </div>
          </div>
        </div>
      )}
