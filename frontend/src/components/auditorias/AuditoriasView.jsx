...
export default function AuditoriasView({ auditorias, setAuditorias, puedeTodasAreas, areaUsuario }) {
  ...
  const getEstadoBadge = (estado) => {
    ...
    return badges[estado] || 'bg-[#00152e]/50 text-slate-300 border-cyan-500/20';
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
        <div className="glass-card-dark p-5 rounded-xl border border-cyan-500/20 shadow-sm relative overflow-hidden">
          <div className="absolute right-0 top-0 w-20 h-20 glass-card-dark-header rounded-bl-full -z-10"></div>
          <p className="text-sm font-bold text-slate-400 mb-1">Total Auditorías</p>
          <p className="text-3xl font-black text-white">{safeAuditorias.length}</p>
        </div>
        <div className="glass-card-dark p-5 rounded-xl border border-emerald-200 shadow-sm relative overflow-hidden">
        ...
        <div className="glass-card-dark p-5 rounded-xl border border-blue-200 shadow-sm relative overflow-hidden">
        ...
        <div className="glass-card-dark p-5 rounded-xl border border-amber-200 shadow-sm relative overflow-hidden">
      ...
      <div className="glass-card-dark rounded-2xl shadow-sm border border-cyan-500/20 overflow-hidden">
        ...
        {verInformes && (
          <div className="p-6 glass-card-dark-header border-b border-cyan-500/20 animate-slide-down">
            {loadingInformes ? (
              <div className="flex flex-col items-center justify-center py-12">
                <Loader2 className="animate-spin text-cyan-500 mb-3" size={32} />
                <span className="font-medium text-slate-400">Conectando con la base de datos...</span>
              </div>
            ) : (
              <>
                <div className="flex flex-wrap gap-2 mb-6 glass-card-dark p-2 rounded-xl border border-cyan-500/20 inline-flex">
                    ...
                    <button
                      ...
                      className={`px-5 py-2 rounded-lg font-bold text-sm transition-all ${
                        anioSeleccionado === anio 
                          ...
                          : 'bg-transparent text-slate-400 hover:bg-[#00152e]/50 hover:text-slate-300'
                      }`}
                    >
                      {anio}
                    </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {informesFiltrados.length === 0 ? (
                    <div className="col-span-full p-8 text-center glass-card-dark rounded-xl border border-dashed border-cyan-500/30">
                      <p className="text-4xl mb-3 opacity-50">📭</p>
                      <p className="font-bold text-slate-300">No hay informes disponibles para el año {anioSeleccionado}</p>
                    </div>
                  ) : (
                    informesFiltrados.map((inf, i) => (
                      <div key={i} className="glass-card-dark p-4 rounded-xl border border-cyan-500/20 flex items-start gap-4 hover:shadow-md transition-shadow group">
                        ...
                        <div className="flex-1">
                          <h4 className="font-bold text-white text-sm leading-tight mb-1">{inf.nombre}</h4>
      ...
      <div className="glass-card-dark rounded-2xl shadow-sm border border-cyan-500/20 overflow-hidden">
        <div className="px-6 py-4 glass-card-dark-header border-b border-cyan-500/20 flex justify-between items-center">
          <h2 className="font-bold text-white flex items-center gap-2">Plan Anual de Auditorías {new Date().getFullYear()}</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50/50">
              <tr>
                <th className="p-4 text-sm font-bold text-slate-300">Número</th>
                <th className="p-4 text-sm font-bold text-slate-300">Área Auditada</th>
                <th className="p-4 text-sm font-bold text-slate-300">Tipo</th>
                <th className="p-4 text-sm font-bold text-slate-300">Fechas Programadas</th>
                <th className="p-4 text-sm font-bold text-slate-300 text-center">Hallazgos</th>
                <th className="p-4 text-sm font-bold text-slate-300">Estado</th>
                <th className="p-4 text-sm font-bold text-slate-300 text-center">Acciones</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-cyan-500/10">
              {safeAuditorias.length === 0 ? (
                ...
                <tr key={aud.id} className="hover:bg-white/5/50 transition-colors group">
                  <td className="p-4 font-mono font-bold text-slate-300 text-sm">{aud.numero}</td>
                  <td className="p-4 font-medium text-white">{aud.area}</td>
                  <td className="p-4">
                    <span className="px-2 py-1 text-[10px] font-bold uppercase bg-[#00152e]/50 text-slate-300 rounded border border-cyan-500/20">
                      {aud.tipo}
                    </span>
                  </td>
                  ...
                  <td className="p-4 text-center">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${aud.hallazgos > 0 ? 'bg-red-100 text-red-600' : 'bg-[#00152e]/50 text-slate-400'}`}>
                      {aud.hallazgos}
                    </span>
                  </td>
                  <td className="p-4">
      ...
      {mostrarModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
          <div className="glass-card-dark rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-slide-up">
            <div className="px-6 py-4 bg-gradient-to-r from-[#002855] to-[#00152e] flex justify-between items-center">
              <h3 className="font-bold text-white flex items-center gap-2"><Plus size={18} /> Programar Auditoría</h3>
            </div>
            
            <div className="p-6 space-y-5">
              <div className="text-center p-6 glass-card-dark-header rounded-xl border border-cyan-500/20 border-dashed">
                <p className="text-slate-400 font-medium mb-4">Esta función creará una nueva auditoría en el sistema.</p>
            ...
            <div className="px-6 py-4 glass-card-dark-header border-t border-slate-100 flex justify-end">
              <button onClick={() => setMostrarModal(false)} className="px-6 py-2.5 border border-cyan-500/20 glass-card-dark font-bold text-slate-300 rounded-lg hover:glass-card-dark-header transition-colors">Cerrar</button>
            </div>
          </div>
        </div>
      )}
