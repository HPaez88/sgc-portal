  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
        ...
        <>
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1.5">Área *</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1.5">Proceso *</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1.5">Número de Auditoría</label>
                <input type="text" name="numero_auditoria" value={form.numero_auditoria} onChange={handleChange}
                  ...
                  className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg disabled:opacity-60 disabled:cursor-not-allowed focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all outline-none" />
              </div>
            </div>
          </div>

          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <h3 className="font-bold text-[#002855] mb-4 border-b pb-2 flex items-center gap-2">⚠️ Descripción de la No Conformidad</h3>
            <textarea name="descripcion_no_conformidad_original" value={form.descripcion_no_conformidad_original} onChange={handleChange}
              rows={4} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all outline-none" placeholder="Describe la no conformidad encontrada de manera detallada..." />
            ...
            <div className="flex gap-6 mt-4">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm font-medium text-slate-700">¿Impacta otros procesos? - SI</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
            ...
            {form.impacta_otros_procesos === 'SI' && (
              <textarea name="otros_procesos_afectados" value={form.otros_procesos_afectados} onChange={handleChange}
                rows={2} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg mt-3 focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all outline-none" placeholder="Especifica qué otros procesos se ven afectados..." />
            )}
          </div>

          <div className="flex gap-3 justify-end border-t pt-4">
            <button onClick={() => setVista('lista')} className="px-6 py-2.5 bg-white border border-slate-200 text-slate-700 font-medium rounded-lg hover:bg-slate-50 transition-colors">
              Cancelar
            </button>
      ...
      {step === 2 && (
        <>
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <h3 className="font-bold text-[#002855] mb-2 border-b pb-2 flex items-center gap-2">👥 Integrantes del Equipo</h3>
                    ...
                    <tr key={integrante.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-2">
                        <input type="text" value={integrante.area} onChange={(e) => actualizarIntegrante(integrante.id, 'area', e.target.value)}
                          className="w-full p-2 bg-white border border-slate-300 rounded focus:border-cyan-500 outline-none transition-all" placeholder="Área" />
                      </td>
                      <td className="p-2">
                      ...
                      <td className="p-2 text-center align-middle">
                        <input type="checkbox" checked={integrante.es_responsable_principal} onChange={(e) => actualizarIntegrante(integrante.id, 'es_responsable_principal', e.target.checked)}
                          className="w-5 h-5 text-cyan-600 rounded border-slate-300 cursor-pointer" />
                      </td>
                      <td className="p-2 text-center align-middle">
      ...
      {step === 3 && (
        <>
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <h3 className="font-bold text-[#002855] mb-4 border-b pb-2">📝 Descripción Mejorada</h3>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <h3 className="font-bold text-[#002855] mb-4 border-b pb-2">🛡️ Acción Contenedora Inmediata</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-4">
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1.5">Actividad Específica</label>
                <input type="text" name="actividad_inmediata" value={form.actividad_inmediata} onChange={handleChange}
                  className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all outline-none" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1.5">Responsable</label>
                <input type="text" name="responsable_actividad_inmediata" value={form.responsable_actividad_inmediata} onChange={handleChange}
                  className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all outline-none" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1.5">Fecha de término</label>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <h3 className="font-bold text-[#002855] mb-4 border-b pb-2 flex items-center justify-between">
            ...
            <div className="overflow-x-auto border border-slate-200 rounded-lg">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {causas.map((c, i) => (
                    <tr key={c.id} className={`${c.es_causa_principal ? 'bg-amber-50' : 'hover:bg-slate-50'} transition-colors`}>
                      <td className="p-2 text-center font-medium text-slate-500">{i + 1}</td>
                      <td className="p-2">
                      ...
                      <td className="p-2">
                        ...
                        }} className="w-full p-2 bg-white border border-slate-300 rounded text-center focus:border-cyan-500 outline-none transition-all" />
                      </td>
                      <td className="p-2 text-center align-middle">
            ...
            <div className="mt-5 bg-slate-50 p-4 border border-slate-200 rounded-lg flex flex-col md:flex-row md:items-center gap-4">
              <label className="font-semibold text-slate-700 whitespace-nowrap">¿Actualizar matriz de riesgos?</label>
              <select name="requiere_actualizar_matriz_riesgos" value={form.requiere_actualizar_matriz_riesgos} onChange={handleChange}
                className="p-2 bg-white border border-slate-300 rounded-lg font-medium w-full md:w-auto">
              ...
              {form.requiere_actualizar_matriz_riesgos === 'SI' && (
                <input type="text" name="descripcion_riesgo_oportunidad" value={form.descripcion_riesgo_oportunidad} onChange={handleChange}
                  className="w-full p-2 bg-white border border-slate-300 rounded-lg focus:border-cyan-500 outline-none" placeholder="Descripción breve..." />
              )}
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                  ...
                  {actividades.map((act, i) => (
                    <tr key={act.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-2">
                        ...
                        }} className="w-full p-2 bg-white border border-slate-300 rounded focus:border-cyan-500 outline-none resize-none" />
                      </td>
                      <td className="p-2">
                        ...
                        }} className="w-full p-2 bg-white border border-slate-300 rounded focus:border-cyan-500 outline-none" />
                      </td>
                      <td className="p-2">
            ...
            <div className="mt-4 flex gap-4 items-center bg-slate-50 p-3 rounded-lg border border-slate-200 w-max">
              <label className="font-medium text-slate-700">¿Requiere cambio en SGC?</label>
          ...
          <div className="flex gap-3 justify-end border-t pt-4">
            <button onClick={() => setStep(2)} className="px-6 py-2.5 bg-white border border-slate-200 text-slate-700 font-medium rounded-lg hover:bg-slate-50 transition-colors mr-auto">
              ← Atrás
            </button>
            <button onClick={guardarBorrador} disabled={loading} className="px-6 py-2.5 bg-white border border-slate-200 text-slate-700 font-medium rounded-lg hover:bg-slate-50 transition-colors">
              {loading ? 'Guardando...' : 'Guardar Borrador'}
