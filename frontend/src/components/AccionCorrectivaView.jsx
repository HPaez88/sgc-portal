...
export default function AccionCorrectivaView({ accionesCorrectivas, setAccionesCorrectivas, usuarios, puedeTodasAreas, areaUsuario, usuarioLogueado }) {
  ...
  // Vista LISTA - mostrar lista de AC
  if (vista === 'lista') {
    return (
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-[#002855]">Acciones Correctivas</h2>
          <button onClick={() => {
            setForm({
              id: null, folio_numero: null, folio_codigo: 'Pendiente de aprobación', anio_folio: null,
              estado: 'BORRADOR', area: areaUsuario || '', proceso: '', origen: '', numero_auditoria: '',
              descripcion_no_conformidad_original: '', impacta_otros_procesos: 'NO', otros_procesos_afectados: '',
            ...
            setVista('nuevo');
          }} className="px-4 py-2 bg-[#002855] text-white rounded-lg hover:bg-[#004a80]">
            + Nueva AC
          </button>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          <input type="text" placeholder="Buscar..." className="px-3 py-2 border rounded-lg text-sm" />
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
          <table className="w-full">
            <thead className="bg-slate-100">
              <tr>
                <th className="p-3 text-left text-sm font-semibold">Folio</th>
                <th className="p-3 text-left text-sm font-semibold">Área</th>
                <th className="p-3 text-left text-sm font-semibold">Estado</th>
                <th className="p-3 text-left text-sm font-semibold">Origen</th>
                <th className="p-3 text-left text-sm font-semibold">Fecha</th>
              </tr>
            </thead>
            <tbody>
              {(!accionesCorrectivas || accionesCorrectivas.length === 0) ? (
                <tr><td colSpan={5} className="p-8 text-center text-slate-500">No hay acciones correctivas. Crea una nueva.</td></tr>
              ) : (
                accionesCorrectivas.map(ac => (
                  <tr key={ac.id} className="border-t hover:bg-slate-50 cursor-pointer" onClick={() => { setForm(ac); setVista('detalle'); }}>
                    <td className="p-3 text-sm">{ac.folio_codigo || 'Sin folio'}</td>
                    <td className="p-3 text-sm">{ac.area}</td>
                    <td className="p-3 text-sm"><span className={`px-2 py-1 rounded text-xs ${ac.estado === 'CERRADO' ? 'bg-green-100 text-green-700' : ac.estado === 'BORRADOR' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'}`}>{ac.estado}</span></td>
                    <td className="p-3 text-sm">{ac.origen}</td>
                    <td className="p-3 text-sm">{ac.fecha_creacion_borrador?.split('T')[0] || '-'}</td>
                  </tr>
                ))
              )}
            </tbody>
  ...
  // Continúa con las demás vistas (nuevo, detalle, etc.)
