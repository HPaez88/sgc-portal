...
setLoading(true);
    ...
    // Si vista es lista, mostrar lista de AC
  if (vista === 'lista') {
    return (
              ...
              {(!accionesCorrectivas || accionesCorrectivas.length === 0) ? (
                ...
                accionesCorrectivas.map(ac => (
                  <tr key={ac.id} className="border-t hover:bg-slate-50 cursor-pointer" onClick={() => { setForm(ac); setVista('detalle'); }}>
                    ...
                    <td className="p-3 text-sm"><span className={`px-2 py-1 rounded text-xs ${ac.estado === 'CERRADO' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>{ac.estado}</span></td>
  ...
  // Vista de formulario (nuevo o detalle)
  return (
